...
def plot_audvis_and_spcorrs_together(result_versions_resp, experiments_resp, conditions_resp, colors_resp,
    ...
    for version_resp, version_spcorr in zip(result_versions_resp,result_versions_spcorr):
        ...
        if 'symmetrical' in version_resp:
            width = 5
        else:
            width = 8
        ...
        for r,roi in enumerate(np.arange(1,len(labels)+1)):
            print('ROI '+str(roi)+': '+labels.iloc[roi-1,0])
            if 'language_standard' in version_resp:
                roi_resp = labels.iloc[roi-1,0]
            else:
                roi_resp = roi
            ...
            fig, ax = plt.subplots(figsize=(width,7),visible=True)
            sliding_x = 1
            xtickvalues = []
            xticklabels = []
            ...
            for e,expt in enumerate(experiments_resp):
                ...
                for i,cond in enumerate(conds_curr): 
                    conds_to_plot[:,i] = data.loc[(data.ROI==roi_resp) & (data.Effect==cond)]['EffectSize']
            ...
            # --- SPCORRS ---
            if 'symmetrical' not in version_resp:
                ...
                for e,expt in enumerate(experiments_spcorr):
                    ...
                    spcorrs = data.loc[(data.ROI==roi)]['Fisher transformed correlation coefficients']
            ...
            # final plot params
            ax.set_xticks(xtickvalues,xticklabels,rotation=45,fontsize=14,ha='right')
            # left,right = plt.xlim()
            ax.plot([0,sliding_x],[0,0],'k')
            ...
            geometric_mean_all_data.to_csv(f'spcorr/spcorrs_summary_{labels.iloc[roi-1,0]}.csv',index=False)
            ax.set_xlim([0,sliding_x])
            ax2.set_ylim([0,0.8])
            ax.set_ylabel('Fisher-transformed Correlation Coefficients',fontsize=18)
            ...
            savename = os.path.join(PLOTS_DIR,"".join([version_no_star,'_ROI_',str(roi),'_',labels.iloc[roi-1,0],'_',title,'_no_scatter.pdf']))
